...
pula = "S" #faz com que o programa vá direto ao processamento dos arquivos
...
def processa_links():
    ...
    with open('links.txt', 'r') as arqv_links:  #abrindo o arqv de links para processar um de cada vez
        for linha in arqv_links:
            ...
            #Checa data
            if (data_html == "S"): #data esta no html
                    if not (data.find("2017") != -1 or data.find("2016") != -1 or data.find("2015") != -1): #anos permitidos
                        dataOK = 1  #noticia nao pode ser processada
                    else:
                        dataOK = 0 #noticia pode ser processada
            ...
            if (dataOK == 0):
                with open(pasta + "/" + 'nots_' + nome_site + '.txt', 'a', encoding='utf-8') as arqv_nots:
                    ...
                    arqv_nots.write(soup.title.string + '\n')  #titulo da noticia
                    arqv_nots.write(data + '\n') #data da noticia
                    if (qntd_param == 1):  #conteudo da pagina, a noticia, e a quantidade de parametros pra extrair tal
                        for item in soup.findAll(param1):  #selecionando os blocos com as noticias
                            arqv_nots.write(item.get_text()) #extraindo e escrevendo cada bloco no arquivo
                    ...
                    else: #qntd de parametros maior que 1
                        for item in soup.findAll(param1,param2):
                            arqv_nots.write(item.get_text()) #extraindo os blocos da noticia
                            arqv_nots.write('\n') #escrevendo os blocos
                            for lin in item.find_all('a'):
                                with open('citados.txt', 'a') as arqv_citados:  #extrai os links citados no corpo das noticias
                                    test = lin.get('href')
                                    if (test is not None):
                                        arqv_citados.write(test + '\n')
                    arqv_nots.write("\nYippieKiYay\n") #Separador
