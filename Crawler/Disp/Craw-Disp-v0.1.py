...
nome_site = "EBC"
START_URL = "http://www.ebc.com.br/noticias"
ER = "\A(http|https)://agenciabrasil\.ebc\.com\.br/.*" #expressao regular para delimitar o dominio do link, delimita a busca somente ao site desejdo
...
def extrai_links(url):
    # extrai os links do portal (pag inicial)
    posts = get_portal_links(url)  # conterá as classes de onde estao as url

    dados = []  # terá os links derivados do link inicial(PORTAL)

    # ver os links q a pag geram
    for post in posts:  # preenchendo a lista com os links
        post_data = extrai_dados(post)  # extraindo os links
        dados.append(post_data)  # colocando apenas os links

    for link in dados:  # percorrendo os links na lista
...
def processa_links():
    ...
    with open('links.txt', 'r') as arqv_links:  #abrindo o arqv de links para processar um de cada vez
        for linha in arqv_links:
            ...
            if (data == "1"): #a pagina nao contem data, logo é invalida, porem ainda temos que extrair as urls dela
                dataOK = False

            #Checa data
            if (dataOK is True and data_html == "S"): #data esta no html e é valida
            ...
            dataOK = True #setando o flag de volta

if (pular == "N"):
    extrai_links(START_URL)
